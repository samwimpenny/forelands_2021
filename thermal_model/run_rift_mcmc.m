function run_rift_mcmc
%%% Runs the 1-D rifting model with a random
%%% sampling of parameters within a given 
%%% parameter range, stores model results 
%%% that fit the subsidence history data 
%%% and calculates the dT,dP,Ss too and 
%%% saves these.
...
z_uc   = 10:1:30;
trift  = 10:1:25;
Huc    = 1e-6:0.1e-6:2.5e-6;
Hlc    = 0.2e-6:0.05e-6:0.8e-6;

% Run time variables:
modelruns = 1000 ;
nrvar     = 8; % number of run variables
gmod      = zeros(modelruns,nrvar+1); % storing 'good' models + misfit
yr2sec    = 60*60*24*365.25;

% Extracting random subset from model ranges
lithr  = lith(randi([1 numel(lith)],modelruns,1));
betar  = beta(randi([1 numel(beta)],modelruns,1));
deltar = delta(randi([1 numel(delta)],modelruns,1));
zcrustr= zcrust(randi([1 numel(zcrust)],modelruns,1));
z_ucr  = z_uc(randi([1 numel(z_uc)],modelruns,1));
triftr = trift(randi([1 numel(trift)],modelruns,1));
Hucr   = Huc(randi([1 numel(Huc)],modelruns,1));
Hlcr   = Hlc(randi([1 numel(Hlc)],modelruns,1));

% Parallel loop for grid search:
for i=1:modelruns
    
    % If delta = 1, then set it to beta
    if deltar(i) == 1.0
        delta_in = betar(i);      
    else
        delta_in = deltar(i);
    ...
    % fprintf('beta = %2.2f and delta = %2.2f \n',betar(i),delta_in)
    [T,z,t,s,zc] = onedrift_td(z_ucr(i),zcrustr(i)-z_ucr(i),lithr(i),betar(i),delta_in,triftr(i),Hucr(i),Hlcr(i));

    % Checking models against constraints
    [flag,misfit] = check_constraints(T,z,t,s,triftr(i)*yr2sec*1e6,zc);
        
    % Storing models that fit constraints
    if flag == 1;
       gmod(i,:) = [z_ucr(i),zcrustr(i)-z_ucr(i),lithr(i),betar(i),delta_in,triftr(i),Hucr(i),Hlcr(i),misfit];
       save_model_output(T,z,t,s,z_ucr(i),zcrustr(i)-z_ucr(i),lithr(i),betar(i),delta_in,triftr(i),Hucr(i),Hlcr(i));
    end
    
end

% Saving gmod
subname = 'output_mcmc/models_match_obs.out';
dlmwrite(subname,gmod,'delimiter','\t')

function [flag,misfit] = check_constraints(T,z,t,s,trift,zc)
%%% Checks whether the constraints imposed
%%% on the temperature structure are 
%%% satisfied by a particular model.

% Constraint 1: T(z<zcrust,tr)=800-950 deg at 
% some point during the rifting phase.
zmin=30e3; zmax=40e3;
ind1 = sum(sum((T>=800).*(T<=900).*(z>=zmin).*(z<=zmax).*(t<=trift).*zc));
if ind1 > 0;
    flag1 = 1;
else
    flag1 = 0;
...
% Constraint 2: Crustal thickness is 
% greater than 30 km in order to match 
% the receiver function observations.
zmin=30e3; zmax=45e3;
zc_final = max(z(:,end).*zc(:,end));
if zc_final >= zmin && zc_final <= zmax;
    flag2 = 1;
else
    flag2 = 0;
end

% Constraint 3: Total sediment-loaded subsidence
% is within the range of the Metan-Alemania Basin
% recorded subsidence history.
sigma      = 300;
misfit_max = 3;

% [time_start, time_end, ds, s_total, s_total_backstrip]
sdata = [100.00 75.80  2.52  2.26  2.52;
          75.80 68.20  0.30  2.52  2.82;
          68.20 61.10  0.17  2.67  2.99;
          61.10 56.30  0.11  2.77  3.11;
          56.30 49.70  0.14  2.90  3.25;
          49.70 40.00  0.26  3.15  3.50;
          40.00 18.50  0.13  3.28  3.63];
 
smodel = interp1(t(1,:)/(60*60*24*365.25*1e6),s,100-sdata(:,2));
misfit = sum(((sdata(:,5)*1e3 - smodel)/sigma).^2);
...
function save_model_output(T,z,t,zsub,zuc0,zlc0,zl0,beta,delta,trift,Hs_uc,Hs_lc)
%%% Saves the model result to output 
%%% directiory for later post-processing.

km2m      = 1e3;
yr2sec    = 60*60*24*365.25;

% Saving temperature-time array as vector:
tt = [t(:)/(yr2sec*1e6) z(:)/km2m T(:)];
fname = ['output_mcmc/temp_' num2str(zuc0,'%2d') '_'  num2str(zlc0,'%2d')  ... 
         '_' num2str(zl0,'%3d') '_' num2str(beta,'%1.2f') ...
         '_' num2str(trift,'%2d') '_' num2str(Hs_uc*1e6,'%2.1f') ...
         '_' num2str(Hs_lc*1e6) '_' num2str(delta,'%1.2f') '.out'];
disp(fname)
dlmwrite(fname,tt,'delimiter','\t')

% Saving subsidence history:
subsidence = [t(end,:)'/(yr2sec*1e6) zsub'/km2m];
subname = ['output_mcmc/subs_' num2str(zuc0,'%2d') '_'  num2str(zlc0,'%2d')  ... 
         '_' num2str(zl0,'%3d') '_' num2str(beta,'%1.2f') ...
         '_' num2str(trift,'%2d') '_' num2str(Hs_uc*1e6,'%2.1f') ...
         '_' num2str(Hs_lc*1e6) '_' num2str(delta,'%1.2f') '.out'];
dlmwrite(subname,subsidence,'delimiter','\t')

function [temptime,zarr,tarr,zsub,zcrust] = onedrift_td(zuc0,zlc0,zl0,beta,delta,trift,Hs_uc,Hs_lc)
...
%%% solved by iteration of the form: A.x^m+1 = b
...
z_nm = 150        ; %[km], Minimum thickness of post-rift lithosphere.

% Model scales:

tend = 100   ; % [Myrs], end of model
...
% Pressure at the base of lithosphere

P1 = trapz(zarr(zarr(:,1)<=zl0,1),rhoarr(zarr(:,1)<=zl0,1));
...
for i = 2:nt
    ...
    if t(i) <= trift
        ...
        zarr(ind_lm,i) = linspace(dzlm,zlm,nzlm)+zc  ;
        zarr(ind_m,i)  = linspace(dz0,zb,nza)+zc+zlm ;
    ...
    if zlm+zc > z_nm && zl0 > z_nm 
        ind_bc = find(zarr(:,i)>=zlm+zc);
    elseif zlm+zc <= z_nm && zl0 > z_nm
        ind_bc = find(zarr(:,i)>=z_nm);
    elseif zlm+zc <= z_nm && zl0 <= z_nm && t(i) < trift
        ind_bc = find(zarr(:,i)>=zl0);
    elseif zlm+zc <= z_nm && zl0 <= z_nm && t(i) >= trift
        ind_bc = find(zarr(:,i)>=zl0);
    end
